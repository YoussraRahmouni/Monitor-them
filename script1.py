...
previous_date=datetime.datetime(2022, 1, 3, 19, 0, 0)


line_parser = apache_log_parser.make_parser("%h %l %u %t \"%r\" %>s %b \"%{Referer}i\" \"%{User-Agent}i\"")

def count_difference():
    last_log=sshcmd("monitorme1.ddns.net", 22, "interfadm", "Projet654!","tail -1 /var/log/apache2/access.log.1")
    ...
    if last_line_date >= previous_date :
        ...
        while log_date > previous_date:
            ...
            log="tail -"+str(n)+" /var/log/apache2/access.log.1 | head -1"
            current_log=sshcmd("monitorme1.ddns.net", 22, "interfadm", "Projet654!",log);
