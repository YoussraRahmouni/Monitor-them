...
#Fonction pour les machines 2 et 3
def getDataM2M3(machine_name, fichier_log):
    line_parser = apache_log_parser.make_parser("%h %l %u %t \"%r\" %>s %b \"%{Referer}i\" \"%{User-Agent}i\"")
    previous_date=datetime.now()-timedelta(minutes=20)
...
#Fonction pour la machine 1        
def getDataM1(machine_name, fichier_log):
    line_parser = apache_log_parser.make_parser("%h %l %u %t \"%r\" %>s %b \"%{Referer}i\" \"%{User-Agent}i\"")
    previous_date=datetime.now()-timedelta(minutes=15)
    last_log=sshcmd(machine_name, 22, "interfadm", "Projet654!",("tail -1 /var/log/apache2/"+fichier_log))
    parsed_last_log=log_parsing(last_log)
    #print(parsed_last_log)
    last_line_date=parsed_last_log["time_received_datetimeobj"]
